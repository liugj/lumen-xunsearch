...
class XSSearch extends XSServer
{
    ...
    public function setAutoSynonyms($value = true)
    {
        ...
        $cmd = ['cmd' => XS_CMD_QUERY_PARSEFLAG, 'arg' => $flag];
        $this->execCommand($cmd);
    ...
    public function getAllSynonyms($limit = 0, $offset = 0, $stemmed = false)
    {
        ...
        $cmd = ['cmd' => XS_CMD_SEARCH_GET_SYNONYMS, 'buf1' => $page];
    ...
    public function getSynonyms($term)
    {
        ...
        $cmd = ['cmd' => XS_CMD_SEARCH_GET_SYNONYMS, 'arg1' => 2, 'buf' => $term];
        $res = $this->execCommand($cmd, XS_CMD_OK_RESULT_SYNONYMS);
        $ret = $res->buf === '' ? [] : explode("\n", $res->buf);
    ...
    public function getQuery($query = null)
    {
        ...
        if (strpos($res->buf, 'VALUE_RANGE') !== false) {
            ...
            $res->buf = preg_replace_callback($regex, [$this, 'formatValueRange'], $res->buf);
        }
    ...
    public function setFacets($field, $exact = false)
    {
        ...
        $cmd = ['cmd' => XS_CMD_SEARCH_SET_FACETS, 'buf' => $buf];
    ...
    public function getFacets($field = null)
    {
        ...
        return isset($this->_facets[$field]) ? $this->_facets[$field] : [];
    }
    ...
    public function setScwsMulti($level)
    {
        $level = intval($level);
        if ($level >= 0 && $level < 16) {
            $cmd = ['cmd' => XS_CMD_SEARCH_SCWS_SET, 'arg1' => XS_CMD_SCWS_SET_MULTI, 'arg2' => $level];
            $this->execCommand($cmd);
        }
    ...
    {
        $name = strval($name);
        $this->execCommand(['cmd' => XS_CMD_SEARCH_SET_DB, 'buf' => strval($name)]);
    ...
    {
        $name = strval($name);
        $this->execCommand(['cmd' => XS_CMD_SEARCH_ADD_DB, 'buf' => $name]);
        $this->_curDbs[] = $name;
     ...
     *
     * @param string $query         搜索语句, 若传入 null 使用默认语句, 最大长度为 80 字节
     * @param bool   $saveHighlight 是否存储查询词用于高亮处理, 默认为 true
     *
    ...
    public function getExpandedQuery($query, $limit = 10)
    {
        ...
        try {
            ...
            $cmd = ['cmd' => XS_CMD_QUERY_GET_EXPANDED, 'arg1' => $limit, 'buf' => $buf];
            $res = $this->execCommand($cmd, XS_CMD_OK_RESULT_BEGIN);
    ...
    public function getCorrectedQuery($query = null)
    {
        ...
        try {
            ...
            $buf = XS::convert($query, 'UTF-8', $this->_charset);
            $cmd = ['cmd' => XS_CMD_QUERY_GET_CORRECTED, 'buf' => $buf];
    ...
    {
        $cmd = ['cmd' => XS_CMD_SEARCH_ADD_LOG, 'buf' => $query];
        if ($wdf > 1) {
            $cmd['buf1'] = pack('i', $wdf);
        }
    ...
    private function cleanFieldQuery($query)
    {
        ...
        if (strpos($query, ':') !== false) {
            ...
            return preg_replace_callback($regex, [$this, 'cleanFieldCallback'], $query);
        }
     ...
     * @param string $value
     *
     * @return float unserialised number
     */
    ...
    /**
     * @return bool whether the user agent is a robot or search engine
     */
    private function isRobotAgent()
    {
        if (isset($_SERVER['HTTP_USER_AGENT'])) {
            $agent = strtolower($_SERVER['HTTP_USER_AGENT']);
            $keys = ['bot', 'slurp', 'spider', 'crawl', 'curl'];
            foreach ($keys as $key) {
