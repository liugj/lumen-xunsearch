...
class XSIndex extends XSServer
{
    ...
    public function del($term, $field = null)
    {
        ...
        if ($this->_bufSize > 0) {
            ...
            $cmd = ['cmd' => XS_CMD_INDEX_EXDATA, 'buf' => implode('', $cmds)];
    ...
    public function addExdata($data, $check_file = true)
    {
        ...
        $cmd = ['cmd' => XS_CMD_INDEX_EXDATA, 'buf' => $data];
        $this->execCommand($cmd, XS_CMD_OK_RQST_FINISHED);
    ...
    {
        $cmd = ['cmd' => XS_CMD_SEARCH_SCWS_GET, 'arg1' => XS_CMD_SCWS_GET_MULTI];
    ...
    {
        $this->execCommand(['cmd' => XS_CMD_INDEX_REBUILD, 'arg1' => 0], XS_CMD_OK_DB_REBUILD);
        $this->_rebuild = true;
    ...
    public function endRebuild()
    {
        if ($this->_rebuild === true) {
            $this->_rebuild = false;
            $this->execCommand(['cmd' => XS_CMD_INDEX_REBUILD, 'arg1' => 1], XS_CMD_OK_DB_REBUILD);
        }
    ...
    {
        try {
            $this->execCommand(['cmd' => XS_CMD_INDEX_REBUILD, 'arg1' => 2], XS_CMD_OK_DB_REBUILD);
            $this->_rebuild = false;
        } catch (XSException $e) {
    ...
    {
        $this->execCommand(['cmd' => XS_CMD_INDEX_SET_DB, 'buf' => $name], XS_CMD_OK_DB_CHANGED);
    ...
    {
        $cmd = ['cmd' => XS_CMD_INDEX_USER_DICT, 'arg1' => 1, 'buf' => $content];
